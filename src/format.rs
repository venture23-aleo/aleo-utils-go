...
use snarkvm_console::{
    prelude::{FromBytes, FromStr, Result, ToBytes},
    program::{Identifier, Literal, Plaintext, Value, U128},
};
...
pub extern "C" fn format_message(
    ...
    if !(1..=MAX_CHUNKS).contains(&target_chunks) {
    ...
    let numbers: Vec<U128<CurrentNetwork>> = match buf
        .chunks_exact(16) // group bytes in chunks of 16 to transform into U128
        .map(|chunk| {
            let array: [u8; 16] = chunk.try_into().unwrap();
            U128::from_bytes_le(&array)
        })
        .collect::<Result<Vec<_>>>()
    {
        Ok(v) => v,
        Err(e) => {
            let mut err_str = String::from("failed to convert bytes to U128: ");
    ...
    let numeric_message = numbers
        .iter()
        .map(|number| {
            // transform an array of 16 u8s into an array of snarkVM u128 plaintext literals
            Plaintext::Literal(Literal::U128(*number), Default::default())
        })
        .collect::<Vec<_>>();

    // build DataChunk
    let number_chunks = numeric_message.chunks_exact(32).collect::<Vec<_>>();
    ...
    while i < target_chunks {
        ...
        for (index, plaintext) in number_chunks[i].iter().enumerate() {
            ...
            chunk_map.insert(
                Identifier::<CurrentNetwork>::from_str(&key).unwrap(),
                plaintext.clone(),
            );
        }
        ...
        data_map.insert(
            Identifier::<CurrentNetwork>::from_str(&key).unwrap(),
            Plaintext::Struct(chunk_map, Default::default()),
        );
        i += 1;
    }
    ...
    let mut output_bytes = output_str.into_bytes();
    // Ensure Vec capacity equals its length before returning to avoid capacity/length mismatch on deallocation
...
#[no_mangle]
pub extern "C" fn formatted_message_to_bytes(
    formatted_message_ptr: *const u8,
    ...
    let formatted_message = unsafe {
        match str::from_utf8(slice::from_raw_parts(
            formatted_message_ptr,
            formatted_message_len,
        )) {
            Ok(val) => val,
            Err(e) => {
                let mut err_str =
                    String::from("failed to rebuild formatted message string from pointer: ");
                err_str.push_str(e.to_string().as_str());
    ...
    // Ensure capacity doesn't exceed length (stabilize for deallocation correctness)
    buf.shrink_to_fit();
